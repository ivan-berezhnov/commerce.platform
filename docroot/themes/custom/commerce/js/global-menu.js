...
(function (Drupal, $) {
   ...
   */
  Drupal.commerceMenu = function ($links, options) {
  ...
  /**
   *
   * @type {{attach: Drupal.behaviors.commerceMenu.attach}}
   */
  Drupal.behaviors.commerceMenu = {
    attach: function (context, settings) {
      var $menuContainer = $('#block-commerce-main-menu');

      $menuContainer.once('commerceMenu').each(function () {
        var $links = $('.menu-level-0 > li > .link-container', $(this));
        if ($links.length > 0) {
          var menu = new Drupal.commerceMenu($links);
          menu.init();
        }
      });
    }
