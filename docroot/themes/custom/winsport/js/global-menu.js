...
(function (Drupal, $) {
   ...
   */
  Drupal.winsportMenu = function ($links, options) {
  ...
  /**
   *
   * @type {{attach: Drupal.behaviors.winsportMenu.attach}}
   */
  Drupal.behaviors.winsportMenu = {
    attach: function (context, settings) {
      var $menuContainer = $('#block-winsport-main-menu');

      $menuContainer.once('winsportMenu').each(function () {
        var $links = $('.menu-level-0 > li > .link-container', $(this));
        if ($links.length > 0) {
          var menu = new Drupal.winsportMenu($links);
          menu.init();
        }
      });
    }
